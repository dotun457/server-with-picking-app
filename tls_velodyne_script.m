...
if (s == 0)
    disp("here -------->\n\n\n");
    disp('Location:');
    tmp = ptCloudIn.Location(1,1301:1313,:);
    disp(tmp(1));
end
...
% if (s == 0)
%     disp(" the index here is ");
%     disp(i);
%     disp(" the angle i here is ");
%     disp(angle(i));
%     disp("affine transform is ");
%     disp(tform_R);
%     tmpCloud = pctransform(curr_img,tform_R); % Save in a cell
%     disp('Location after transform by VM');
%     tmp = tmpCloud.Location(1,1301:1313,:);
%     disp(tmp);
% end
...
% 16 corresponds to the band, 1800 corresponds to the number of points recorded per band, 3 corresponds to the x, y and z values.

frame = Cloud{1, ii}; %here frame dimensions are 16 * 1800 * 3
x1(i,:) = frame.Location(i,:,1); %all x coordinates from band i
y1(i,:) = frame.Location(i,:,2);
z1(i,:) = frame.Location(i,:,3);
int1(i,:) = frame.Intensity(i,:);

x = [x x1(i,:)]; %append them to get all x coordinates for frame ii i.e band0 x, band1 x all together
y = [y y1(i,:)];
z = [z z1(i,:)];
int = [int int1(i,:)];
    ...
    % x here is all x coordinates from all bands of a single frame
    % X_ref here is the collation of x coordinates of a set number of
    % frames; if there are 700 frames for the pcd then X_ref will be a list
    % of 70 x coordinates
    X_ref = [X_ref x]; 
    Y_ref = [Y_ref y];
    Z_ref = [Z_ref z];
    ...
    % will run 10 times 
    % X_ref_final will have 10 elements
    % each element is a X_ref with 70 entries of x, each entry of x is from a frame
    % each x is an array of all the coordinates from 32 bands of a single
    % frame
...
cd('/Users/dotunmac/DEV/bdi_project/Velodyne-PCAP-to-Point-Cloud-for-TLS/Matlab 2022a code/input/')
